...
import { Search, ShoppingCart, Plus, Minus, Camera, Calendar } from "lucide-react";
import { RentalEquipment } from "@/lib/types";
import { RentalCheckoutSheet } from "@/components/rentals/RentalCheckoutSheet"; // We create this next
...
export default function RentalsPage() {
  ...
  const [cart, setCart] = useState<{ [id: number]: number }>({}); // ID -> Quantity mapping
  ...
  return (
    <div className="relative min-h-screen flex flex-col">
      ...
      {/* Hero Section */}
      <section className="pt-24 pb-12 px-4 text-center" data-aos="fade-up">
        <h1 className="text-4xl md:text-6xl font-bold mb-4">Evoke Rentals</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto text-lg">
          Professional equipment for your creative productions. 
          Browse our fleet and book what you need.
        </p>
      </section>

      {/* Search & Filter Bar */}
      <div className="container mx-auto px-4 sticky top-20 z-30 mb-8" data-aos="fade-up" data-aos-delay="100">
      ...
      <div className="container mx-auto px-4 pb-32 flex-1">
        {loading ? (
          <div className="text-center py-20 text-muted-foreground">Loading fleet...</div>
        ) : (
                      ...
                      <Card key={item.id} className={`flex flex-col border-white/10 bg-card/40 backdrop-blur-sm transition-all duration-300 ${cart[item.id] ? 'border-primary/50 shadow-[0_0_15px_rgba(37,99,235,0.1)]' : ''}`}>
                        <CardHeader className="p-4 pb-2">
                          <div className="flex justify-between items-start">
                            ...
                            <span className="font-mono text-xs text-muted-foreground">{item.quantity_total} in fleet</span>
                          </div>
      ...
      {/* Floating Cart Bar (Only visible if items selected) */}
      {cartTotalItems > 0 && (
