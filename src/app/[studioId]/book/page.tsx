...
const bookingSchema = z.object({
  ...
  event_date: z
    .string()
    .refine((date) => new Date(date) >= new Date(new Date().toDateString()), { // Allow today or future
...
export default function BookingPage() {
  ...
  }, [selectedEventType, studioName, form, prefilledCategory, prefilledPackage]); // Dependencies
  ...
  async function onSubmit(values: z.infer<typeof bookingSchema>) {
    ...
    try {
      ...
      toast.success("Inquiry Submitted!", { description: "We'll get back to you soon." });
  ...
  return (
    <div className="container mx-auto py-16 px-4 max-w-2xl">
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 bg-card p-6 md:p-8 rounded-lg shadow-sm"> {/* Added card styling */}
          {/* --- Full Name --- */}
          <FormField
               ...
               render={({ field }) => (
                 <FormItem>
                  ...
                  >
                     <FormControl>
                       <SelectTrigger>
                         <SelectValue placeholder={packages.length > 0 ? "Select package" : "Select event type first"} />
                       </SelectTrigger>
                     </FormControl>
                     <SelectContent>
                       {packages.map(pkg => <SelectItem key={pkg.id} value={pkg.name || ''}>{pkg.name} - {pkg.price}</SelectItem>)} {/* Show price */}
                     </SelectContent>
                   </Select>
                   <FormMessage />
                 </FormItem>
